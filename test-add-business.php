...
if (class_exists('ActionScheduler')) {
    ...
    echo "Pending review jobs: " . count($pending_actions) . "\n";
    ...
    echo "Completed review jobs: " . count($completed_actions) . "\n";
    ...
    echo "Failed review jobs: " . count($failed_actions) . "\n";
    
    // Try to manually process some pending actions
    if (count($pending_actions) > 0) {
        echo "\nAttempting to manually process pending jobs...\n";
        ...
        foreach ($pending_actions as $action) {
            ...
            try {
                $result = do_action($action->get_hook(), ...$action->get_args());
                echo "Processed job ID: " . $action->get_id() . "\n";
                $processed++;
            } catch (Exception $e) {
                echo "Failed to process job ID: " . $action->get_id() . " - " . $e->getMessage() . "\n";
            }
        }
        
        echo "Manually processed {$processed} jobs\n";
    }
