...
class Trustpilot_Business_Manager {
    ...
    /**
     * Get existing review hashes for a business domain (for batched deduplication)
     *
     * @param string $business_domain Business domain
     * @return array Array of review hashes that already exist
     */
    private function get_existing_review_hashes($business_domain) {
        global $wpdb;

        // Get term ID for business domain
        $term = get_term_by('slug', $business_domain, 'tp_business');
        if (!$term || is_wp_error($term)) {
            return array(); // No term = no reviews yet
        }

        // Single efficient query: get all review_id meta values for reviews in this business
        $query = $wpdb->prepare("
        ...
        $hashes = $wpdb->get_col($query);
        return $hashes ? $hashes : array();
    }
    ...
    private function create_single_review($review_data, $term) {
        // Extract review hash for regional-safe deduplication
        $review_hash = Trustpilot_Utils::extract_review_hash($review_data['review_id']);

        // Check if review already exists (deduplication by hash)
        ...
        if (!empty($existing_reviews)) {
            // Review already exists, skip duplicate
            return array(
                'success' => true,
                'message' => 'Review already exists (skipped)',
        ...
        // Save review metadata (store hash only for regional-safe deduplication)
        update_post_meta($review_id, 'review_id', $review_hash);
    ...
    public function create_business($trustpilot_url) {
        ...
        try {
            ...
            if ($this->find_business_by_url($trustpilot_url) !== null) {
    ...
    public function process_business_data($business_id, $scraped_data = null) {
        ...
        try {
            ...
            // Queue individual review jobs (with batched deduplication check)
            if (!empty($scraped_data['reviews']) && is_array($scraped_data['reviews'])) {
                $total_scraped = count($scraped_data['reviews']);
                Trustpilot_Utils::debug_log("Trustpilot Debug: Found {$total_scraped} reviews for business {$business_id}");

                // Batch check: get all existing review hashes for this business (1 query)
                $existing_hashes = $this->get_existing_review_hashes($business_domain);
                $skipped_count = 0;

                foreach ($scraped_data['reviews'] as $index => $review) {
                    // Extract hash and check if already exists
                    $review_hash = Trustpilot_Utils::extract_review_hash($review['review_id']);

                    if (in_array($review_hash, $existing_hashes)) {
                        // Already exists, skip queuing this job
                        $skipped_count++;
                        continue;
                    }

                    // New review, queue job
                ...
                Trustpilot_Utils::debug_log("Trustpilot Debug: Queued {$results['reviews_queued']} new review jobs, skipped {$skipped_count} existing reviews for business {$business_id}");
            } else {
